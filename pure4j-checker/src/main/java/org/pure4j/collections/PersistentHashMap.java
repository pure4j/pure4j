...
public class PersistentHashMap extends APersistentMap implements
		IEditableCollection, IObj, IMapIterable, IKVReduce {
	...
	public Object fold(long n, final IFn combinef, final IFn reducef,
		...
		Callable top = new Callable() {
			public Object call() throws Exception {
				...
				if (root != null)
					ret = combinef.invoke(ret, root.fold(combinef, reducef,
							fjtask, fjfork, fjjoin));
				return hasNull ? combinef.invoke(ret,
						reducef.invoke(combinef.invoke(), null, nullValue))
						: ret;
			}
		};
	...
	static interface INode extends Serializable {
		...
		Object fold(IFn combinef, IFn reducef, IFn fjtask, IFn fjfork,
				IFn fjjoin);
	...
	final static class ArrayNode implements INode {
			...
			for (final INode node : array) {
				if (node != null) {
					tasks.add(new Callable() {
						public Object call() throws Exception {
							return node.fold(combinef, reducef, fjtask, fjfork,
									fjjoin);
						}
					});
				}
		...
		static public Object foldTasks(List<Callable> tasks,
				final IFn combinef, final IFn fjtask, final IFn fjfork,
				final IFn fjjoin) {
			...
			final List<Callable> t2 = tasks.subList(tasks.size() / 2,
					tasks.size());
			...
			return combinef.invoke(
					foldTasks(t1, combinef, fjtask, fjfork, fjjoin),
